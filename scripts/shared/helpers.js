...
async function getExSigner(idx) {
  try {
    const accounts = await ethers.getSigners();
    const signer = accounts[idx]
    return signer;
  } catch (e) {
    throw new Error(`getExSigner error: ${e.toString()}`);
  }
}
...
async function deployProxy(name, label, options, registerName, initializer, initializerParams) {
    if (!options && typeof label === "object") {
      label = null;
      options = label;
    }
    ...
    const contractFactory = await ethers.getContractFactory(name, options);
    const proxy = await upgrades.deployProxy(contractFactory, [...initializerParams], {initializer: initializer});
  
    const argStr = initializerParams.map((i) => `"${i}"`).join(" ");
    console.info(`Deploying Proxy ${info} ${proxy.address} ${argStr}`);
    const deployed = await proxy.deployed();
    const ret = await deployed.deployTransaction.wait();
