...
contract PairFactory is IPairFactory, OwnableUpgradeable {
    ...
    function getAllPairs(uint256 startIndex, uint256 endIndex) external view returns (address[] memory) {
        if (endIndex <= startIndex || endIndex > allPairs.length) endIndex = allPairs.length;
        require(startIndex < endIndex, "Nothing to get");

        address[] memory ret = new address[](endIndex - startIndex);
        uint256 i;
        for (i = startIndex; i < endIndex; i ++) {
            ret[i - startIndex] = allPairs[i];
        }
